...
import {useRoute} from "../../router/EmployessRouterContext";
...
export const AdminLayout = () => {
    ...
    const authUserQuery = useFetchQuery('employess.users.getAuthUser', params, 'authUser');
    const tasksQuery = useFetchQuery('employess.tasks.tasks', params, 'tasks');
    ...
    const items = [
        {
            ...
            command: () => {
                navigate(`/employess/dashboard`, {replace: true});
            },
            style: {backgroundColor: location.pathname.startsWith('/employess/dashboard') ? '#EC1C24' : ''}
        },
        {
            label: <label className={'text-white'}>Perfil</label>,
            icon: <img src={'/assets/Administrador/icons/users.svg'} className={'sidebar-icon'}/>,
            command: () => {
                navigate(`/employess/users`, {replace: true});
            },
            style: {backgroundColor: location.pathname.startsWith('/employess/users') ? '#EC1C24' : ''}
        },
        {
            label: <label className={'text-white'}>Tareas</label>,
            icon: <img src={'/assets/Administrador/icons/alert.svg'} className={'sidebar-icon'}/>,
            command: () => {
                navigate(`/employess/tasks`, {replace: true});
            },
            style: {backgroundColor: location.pathname.startsWith('/employess/tasks') ? '#EC1C24' : ''}
        },
    ];
        ...
        try {
            SwalReact.fire({
                ...
                if (result.isConfirmed) {
                    axios.post(route("employess.auth.logout"))
                        .then(response => {
                            window.location.replace('/login/employess');
                        })
                        .catch(error => {
                            console.error(error);
                        });
            ...
            SwalReact.fire({
                ...
                window.location.replace('/employess/dahboard');
            })
        }
    };
                ...
                <Col xs={isSidebarCollapsed ? 11 : 10}>
                    <Row className='justify-content-end'>
                        <Col xs={12} className={'center-content header-user'}>
                            {/* Campana de notificación con número de tareas */}
